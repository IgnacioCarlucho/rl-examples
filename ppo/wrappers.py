...
class EpisodicLifeEnv_v2(gym.Wrapper):
    def __init__(self, env=None):
        """Make end-of-life == end-of-episode, but only reset on true game over.
        Done by DeepMind for the DQN and co. since it helps value estimation.
        """
        super(EpisodicLifeEnv, self).__init__(env)
        self.total_lives = env.unwrapped.ale.lives()
        self.lives = 0
    ...
    def _step(self, action):
        ...
        if lives < self.self.total_lives and lives > 0:
    ...
    def _reset(self):
        ...
        if self.was_real_done:
            print('we want to kill you')
            self.env.unwrapped.ale.game_over()
            self.env.unwrapped.ale.reset_game()
            obs = self.env.reset()
...
class FrameStack_new(gym.Wrapper):
    """
